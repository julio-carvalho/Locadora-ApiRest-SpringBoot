...
public class FilmeController {
	...
	private ResponseEntity<?> alugaFilme(@PathVariable Long id, @RequestBody ClienteDTO clienteDTO) {
		Optional<Filme> buscaFilmeId = filmeService.findById(id);
		...
		filmeService.updateDisponivelFalse(buscaFilmeId.get());
		return ResponseEntity.ok().body(buscaFilmeId);
	}
	
	@PostMapping("/devolver/{id}")
	private ResponseEntity<?> devolveFilme(@PathVariable Long id, @RequestBody ClienteDTO clienteDTO) {
		Optional<Filme> buscaFilmeId = filmeService.findById(id);
